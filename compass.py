...
def initParser():
    ...
    parser.add_argument('scenario', help="Number of scenario", type=int, choices=[3100, 3210, 5400, 23400, 24300])
...
if __name__ == "__main__":
    ...
    title_div = Div(text=f'<h1 style="text-align: center"> COMPASS Upgrade</h1><p>Scenario: {args.scenario} </p><p>Author: Marcin Kastek</p>')
    ...
    src_Ne_2D = ColumnDataSource(data={'x':[0.25],'y':[-1],'dw':[1.25],'dh':[2],'im':[data_2D_Ne[1]]})
    ...
    output_file(filename=os.path.join("output-data",f"COMPASS_scenario_{args.scenario}.html"), title="COMPASS")
